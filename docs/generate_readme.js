...
if (!apiKey) {
  console.error('apiKey not provided, so statistics won\'t be generated')
} else {
  ...
  const count = parseInt(await hexToCssFilterLibrary.queryDb(
    `SELECT COUNT() FROM ${tableName}`,
    { getFirstValue: true }
  ))

  const perfects = parseInt(await hexToCssFilterLibrary.queryDb(
    getLossesEqualToQuery(0),
    { getFirstValue: true }
  ))
  const below01s = parseInt(await hexToCssFilterLibrary.queryDb(
    getLossesBetweenQuery(0, 0.1),
    { getFirstValue: true }
  )) - perfects
  const below02s = parseInt(await hexToCssFilterLibrary.queryDb(
    getLossesBetweenQuery(0.1, 0.2),
    { getFirstValue: true }
  ))
  const below03s = parseInt(await hexToCssFilterLibrary.queryDb(
    getLossesBetweenQuery(0.2, 0.3),
    { getFirstValue: true }
  ))
  const below04s = parseInt(await hexToCssFilterLibrary.queryDb(
    getLossesBetweenQuery(0.3, 0.4),
    { getFirstValue: true }
  ))
  const below05s = parseInt(await hexToCssFilterLibrary.queryDb(
    getLossesBetweenQuery(0.4, 0.5),
    { getFirstValue: true }
  ))
  const below06s = parseInt(await hexToCssFilterLibrary.queryDb(
    getLossesBetweenQuery(0.5, 0.6),
    { getFirstValue: true }
  ))
  const below07s = parseInt(await hexToCssFilterLibrary.queryDb(
    getLossesBetweenQuery(0.6, 0.7),
    { getFirstValue: true }
  ))
  const below08s = parseInt(await hexToCssFilterLibrary.queryDb(
    getLossesBetweenQuery(0.7, 0.8),
    { getFirstValue: true }
  ))
  const below09s = parseInt(await hexToCssFilterLibrary.queryDb(
    getLossesBetweenQuery(0.8, 0.9),
    { getFirstValue: true }
  ))
  const below1s = parseInt(await hexToCssFilterLibrary.queryDb(
